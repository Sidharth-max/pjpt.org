...
export default function Events() {
  ...
  // We are replacing the dynamic featured event with the new hardcoded Dhwajarohan Utsav 2026.
  // We also mix backend non-featured events with the new upcoming ones.
    ...
    <div className="w-full pt-20">
      ...
      <section className="py-20 px-4 text-center bg-white relative">
        <motion.div initial="hidden" animate="visible" variants={fadeUp} className="relative z-10 max-w-3xl mx-auto">
          ...
          <p className="font-cormorant text-xl text-text-muted">
            Join thousands of devotees in sacred observances throughout the year
          </p>
        </motion.div>
      </section>
        ...
        <div className="max-w-[1000px] mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="relative bg-white border border-gold-primary shadow-sm overflow-hidden flex flex-col md:flex-row">
            ...
            <div className="p-8 md:p-12 w-full md:w-3/5 flex flex-col justify-center relative">
              ...
              <h2 className="font-cinzel text-3xl md:text-4xl text-text-dark mb-1">{hardcodedFeaturedEvent.title}</h2>
              ...
              <div>
                <Link to={hardcodedFeaturedEvent.link} className="btn-gold inline-block uppercase text-sm">View Full Details &rarr;</Link>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <>
          <section className="py-20 bg-white px-4">
            <div className="max-w-[1200px] mx-auto">
              <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="font-cinzel text-3xl text-center mb-16 text-text-dark uppercase tracking-wide">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {allRegularEvents.map((event) => (
                  <motion.div key={event._id} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeUp} className="card group overflow-hidden flex flex-col">
                    ...
                    <h3 className="font-cinzel text-xl text-text-dark mb-1 leading-snug">{event.title}</h3>
                    {event.hindi && <h4 className="font-noto text-lg text-gold-primary mb-3">{event.hindi}</h4>}
                    {!event.hindi && <div className="mb-3"></div>}
                    
                    <p className="font-cormorant text-text-muted leading-relaxed flex-grow">
                      {event.description}
                    </p>
                    {event.location && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {recurringEvents.map((event, idx) => (
              <motion.div key={idx} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeUp} className="card bg-white group border-gold-pale">
                ...
                <h3 className="font-cinzel text-lg text-gold-primary mb-3 leading-snug">{event.title}</h3>
                <p className="font-cormorant text-text-muted leading-relaxed">
                  {event.description}
                </p>
              </motion.div>
            ))}
      ...
      <section className="py-24 px-4 bg-white text-center relative overflow-hidden">
         ...
         <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="relative z-10 max-w-2xl mx-auto">
           <h2 className="font-cinzel text-3xl mb-6 text-text-dark uppercase tracking-wide">Join Our Spiritual Community</h2>
           <p className="font-cormorant text-xl text-text-muted mb-10">
             To stay informed about upcoming events, pujas, and volunteering opportunities, please contact the temple office.
           </p>
